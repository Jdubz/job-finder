...
def get_pytest_collection() -> Dict[str, Any]:
    ...
    Returns:
        Dict with collection results including test counts and errors
    """
    result = {
        "collected_tests": 0,
        "errors": 0,
...
def scan_test_files() -> Dict[str, List[Path]]:
    ...
    Returns:
        Dict categorizing files by type
    """
...
def generate_markdown_report(collection: Dict[str, Any], files: Dict[str, List[Path]]) -> str:
    ...
    if files['helper_files']:
        ...
        for file_path in sorted(files['helper_files']):
            purpose = "Package marker" if file_path.name == "__init__.py" else "Pytest fixtures"
...
def main():
    ...
    print(f"E2E files: {len(files['e2e_runners']) + len(files['e2e_scenarios']) + len(files['e2e_helpers'])}", file=sys.stderr)
    print("=" * 60, file=sys.stderr)
