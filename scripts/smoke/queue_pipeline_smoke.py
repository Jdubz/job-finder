...
class SmokeTestRunner:
        ...
        for json_file in json_files:
            try:
                with open(json_file, "r") as f:
                    ...
                    required_fields = ["title", "company", "company_website", "location", 
                                     "description", "url"]
                    missing_fields = [f for f in required_fields if f not in fixture]
                    if missing_fields:
                        logger.warning(
                            f"Fixture {json_file.name} missing fields: {missing_fields}"
                        )
                        continue
    ...
    def submit_jobs(self, jobs: List[Dict[str, Any]]) -> int:
        ...
        for job in jobs:
            try:
                ...
                count = self.scraper_intake.submit_jobs(
                    [job_data],
                    source="smoke_test"
                )
                ...
                if count > 0:
                    ...
                    if docs:
                        ...
                        logger.info(
                            f"Submitted: {job_data.get('title')} (doc_id: {doc_id})"
                        )
    ...
    def poll_until_complete(
        ...
        while pending_urls and (time.time() - start_time) < timeout_seconds:
            for url in list(pending_urls):
                ...
                try:
                    doc = self.queue_manager.db.collection("job-queue") \
                        .document(doc_id).get()
        ...
        if pending_urls:
            logger.warning(
                f"Timeout reached with {len(pending_urls)} jobs still pending"
            )
        ...
        for result in results:
            if result.get("status") == QueueStatus.SUCCESS:
                ...
                try:
                    ...
                    matches = self.job_storage.db.collection("job-matches") \
                    ...
                    missing_fields = [
                        f for f in required_fields if f not in match_data
                    ]
    ...
    def generate_report(
        ...
        avg_time = (
            sum(r.get("elapsed_seconds", 0) for r in results) / total_jobs
            if total_jobs > 0 else 0
        )
        
        # Create markdown report
        markdown_lines = [
            f"# Queue Pipeline Smoke Test Report",
            f"",
...
def main():
    ...
    parser.add_argument(
        ...
        help="Directory containing fixture JSON files "
             "(default: tests/fixtures/smoke_jobs)",
    )
